...
// Fail Fast: Los tests fallan si no están configuradas las variables de entorno
// ============================================================================

/**
 * Obtiene una variable de entorno requerida para pruebas.
 * Si no está definida, termina el proceso con un mensaje de error claro.
 * 
 * SEGURIDAD: Implementa patrón "Fail Fast" para evitar hardcodear credenciales
 * y obligar a configurar el entorno de prueba correctamente.
 */
function getRequiredTestEnv(name: string, description: string): string {
  const value = process.env[name];
  if (!value) {
    console.error(`\n${'='.repeat(80)}`);
    console.error(`❌ ERROR DE CONFIGURACIÓN DE PRUEBAS`);
    console.error(`${'='.repeat(80)}`);
    console.error(`Variable de entorno requerida: ${name}`);
    console.error(`Descripción: ${description}`);
    console.error(`\nPor favor, configure las credenciales de prueba en .env.test o .env:`);
    console.error(`  ${name}=<valor>`);
    console.error(`\nEjemplo de ejecución:`);
    console.error(`  # Windows PowerShell`);
    console.error(`  $env:${name}="valor"; npx ts-node tests/test-hu-jz-001-security.ts`);
    console.error(`\n  # Linux/Mac`);
    console.error(`  ${name}=valor npx ts-node tests/test-hu-jz-001-security.ts`);
    console.error(`${'='.repeat(80)}\n`);
    process.exit(1);
  }
  return value;
...
// Credenciales de prueba desde variables de entorno (SEGURIDAD: No hardcodear)
const CREDENTIALS = {
  juez1: {
    correo: process.env.TEST_JUEZ1_EMAIL || 'emily.luna@judicatura.gob.ec',
    password: getRequiredTestEnv('TEST_JUEZ1_PASSWORD', 'Contraseña del Juez 1 para pruebas de seguridad'),
  },
  juez2: {
    correo: process.env.TEST_JUEZ2_EMAIL || 'said.luna01@judicatura.gob.ec',
    password: getRequiredTestEnv('TEST_JUEZ2_PASSWORD', 'Contraseña del Juez 2 para pruebas de seguridad'),
  },
  admin: {
    correo: process.env.TEST_ADMIN_EMAIL || 'emily.luna@judicatura.gob.ec',
    password: getRequiredTestEnv('TEST_ADMIN_PASSWORD', 'Contraseña del Admin para pruebas de seguridad'),
  },
};
